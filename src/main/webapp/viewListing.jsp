...
<body>
	...
	<p>
		Initial Price:
		<%=rs.getDouble("initial_price")%></p>
	<%--  <p>Minimum Sale Price: <%= rs.getDouble("min_sale") %></p> THIS STAYS HIDDEN!--%>
	<p>
		Closing Date/Time:
		<%=rs.getTimestamp("date_time").toString()%></p>
	<p>
		Status:
		<%=rs.getInt("open_close") == 0 ? "Open" : "Closed"%></p>
	<%
	...
	<form method="post" action="verifyBid.jsp">
		<table>
			<tr>
				<td>Price:</td>
				<td><input type="text" name="price" value="0" maxlength="30"
					required /></td>
				<td><input type="hidden" name="listing_id"
					value='<%=rs.getInt("listing_id")%>' /> <input type="submit"
					value="Place" /></td>
			</tr>
		</table>
	</form>
	...
	<form method="post" action="verifyAutoBid.jsp">
		<table>
			<tr>
				<td>Increments: <input type="text" name="increments" value="0"
					maxlength="30" required /></td>
				<td>Upper Limit: <input type="text" name="upperLimit" value="0"
					maxlength="30" required /></td>
				<td>
					<%-- <%
